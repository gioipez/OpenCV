...
from IntelligentScissors.scissors import search
from IntelligentScissors.scissors.utils import unfold, create_spatial_feats, flatten_first_dims, quadratic_kernel, preprocess_image
...
class CostProcessor:
    ...
    def compute_dynamic_cost(self, series: Sequence[tuple]) -> np.array:
        ...
        total_cost = total_cost.squeeze(0).astype(np.int32)
        return total_cost
    ...
    def get_direct_pixel_feats(image: np.array, n_values: int) -> np.array:
        ...
        local_feats = np.ceil((n_values - 1) * local_feats).astype(np.int32)
        local_feats = unfold(local_feats[None]).astype(np.int32)
        return local_feats
    ...
    def get_externals_pixel_feats(image: np.array, n_values: int, k_distance: int, eps=1e-4) -> np.array:
        ...
        def get_shifted_feats(directions):
            shifts = np.round(directions * k_distance).astype(np.int32)
            ...
            feats = unfold(feats[None]).astype(np.int32)
            return feats
    ...
    def get_magnitude_features(image: np.array, n_values: int, std: int) -> np.array:
        ...
        cost = np.ceil((n_values - 1) * cost).astype(np.int32)
...
class Scissors:
    def __init__(self, image: np.array, capacity=None, use_dynamic_features=True):
        ...
        self.static_cost = static_cost.astype(np.int32)
    ...
    def calculate_segment(seed_x: int, seed_y: int, free_x: int, free_y: int, maximum_cost: int,
        ...
        if dynamic_cost is None:
            dynamic_cost = np.zeros((3, 3, h, w), dtype=np.int32)
